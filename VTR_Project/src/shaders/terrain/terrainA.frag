...
uniform sampler2D rock1, rock2, rock3, rock4, aforest3, snow1, water1, water2, water3, sand1, forest1;
...
void main() {
    ...
    float topThreshold = 2.1;
    float mediumThreshold = 1.5;
    float lowThreshold = 0.35;
    float waterThreshold = 0.1;

    vec4 grassColor = texture(aforest3, tc);
    vec4 snowColor = texture(snow1, tc);
    vec4 sandColor = texture(sand1, tc);
    vec4 forestColor = texture(forest1, tc);

    vec4 rockColor = mix(texture(rock1, tc), texture(rock2, tc), 0.33);
    rockColor = mix(rockColor, texture(rock3, tc), 0.33);
    rockColor = mix(rockColor, texture(rock4, tc), 0.33);
