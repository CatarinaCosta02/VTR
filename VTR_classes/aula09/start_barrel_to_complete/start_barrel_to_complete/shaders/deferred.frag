...
void main() {
    ...
    // Ir buscar informação às texturas
    vec3 n = texture(normal, tc).xyz;
    // quer dizer que nao pintou e fazemos um discard do pixel
    if n == vec3(0.0, 0.0, 0.0) {
        discard;
        return;
    }
    // normal varia entre -1 e 1
    n = normalize (n * 2 - 1);
    ...
    // tc_obj vai ter a coordenada de textura do barril, que está na posição x
    ...
    float noise = clamp(pow((perlinNoise(p.xyz)+0.5)*specMap * 1.5, 4),0,1);
    ...
    if (intensity > 0) {
        ...
        vec4 specMap = texture(speculatMap, tc_obj);
        spec = specMap * pow(specInt, 77);
    }
